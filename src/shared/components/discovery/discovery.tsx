import React, { useEffect, Suspense, lazy } from "react";
import { useNavigate } from "react-router-dom";
import CN from "classnames";
import SCSS from "./discovery.module.scss";
import SearchBar from "../search-bar/search-bar";
import { useAppDispatch, useAppSelector } from "../../../core/redux-store/hooks";
import {
  FetchProminentLocationsAsync, FetchEventsAsync, selectDiscovery, selectEvents, selectProvinces, selectStatus, setSelectedEvent
} from "../../../core/redux-store/features/discovery/discoverySlice";
import { Layout } from "../../../core/layout/layout";
import Title from "../../../core/header/location-title/title";
import { ProminentLocation } from "../../../core/redux-store/features/api/mock-data/simplemaps-locations";
import LocationCards from "./location-cards/location-cards";
import LocationEventCards from "../location-events/location-event-card/location-event-card";
import { LocationEvent } from "../../../core/redux-store/features/api/mock-data/events";

export default function Discovery<React>() {
  const discoveryState = useAppSelector(selectDiscovery);
  const status = useAppSelector(selectStatus);
  const provinces: ProminentLocation[] = useAppSelector(selectProvinces);
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  const events: LocationEvent[] | null = useAppSelector(selectEvents)

  const handleCardClick = (place: ProminentLocation) => {
    navigate(`/discovery/province/${place.admin_name.toLocaleLowerCase()}`);
  };

  const handleEventCardClick = (event: LocationEvent) => {
    dispatch(setSelectedEvent(event))
  }

  // Check store and update it if nothing is there
  useEffect(() => {
    if (status !== "complete") {
      dispatch(FetchProminentLocationsAsync());
      dispatch(FetchEventsAsync())
    }
  }, [])

  useEffect(() => {
    console.log("COMP Discovery useEffet", provinces)
  }, [status])

  return (
    <Layout>
      <Title text="Discovery" />
      <SearchBar />

      {events && <LocationEventCards events={events} handleEventCardClick={handleEventCardClick} />}
      {provinces && provinces.length > 0 && <LocationCards locations={provinces} handleCardClick={handleCardClick} showcase={"province"} />}
    </Layout>

  )
}

function rootReducer(rootReducer: any) {
  throw new Error("Function not implemented.");
}

